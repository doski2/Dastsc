...
--If this is the first run then delete the getdata.txt and sendcommand.txt files
	...
	if counter >= delay then
		if Call ("GetIsEngineWithKey") == 1 then --Check we are driving the train.
			...
			-- Importante: GetSpeedInfo() determina speedoType, pero gData se RE-INICIALIZA abajo
			GetSpeedInfoSimple() 
			...
			-- Reiniciamos gData con los valores base
			gData = string.format("Speed:%.2f|SimulationTime:%.2f|Gradient:%.2f|TrainLength:%.2f|TrainMass:%.2f|TractiveEffort:%.2f", 
				speedMPS * currentFactor, simTime, grad, length, mass, effort) -- Final unit based on train type

			GetSpeedInfo() -- Ahora sí añadimos los detalles de velocidad a gData (CabSpeed, CurrentSpeed, MaxSpeed)
			...
			GetSignalData () -- Get next signal state and distance.
