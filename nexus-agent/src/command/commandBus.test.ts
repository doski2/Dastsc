...
describe('commandBus', () => {
  ...
  it('does not release OFF when stopped at station without plan', () => {
    const snapshot = createMockSnapshot({
      brake: { combined: -0.5, cylinder: 2, effortKn: 40, projectedStopM: 5 },
      speedMs: 0.1,
      speedDisplay: 0,
  ...
  it('holds S1 at platform for split ICE T profile', () => {
    ...
    const snapshot = createMockSnapshot({
      brake: { combined: 0, cylinder: 0, effortKn: 0, projectedStopM: 50 },
      speedMs: 0.1,
      speedDisplay: 0,
      station: { distanceM: 0, nameOcr: 'Test', eta: '' },
      limits: { effective: 25, frontal: 25, next: null, upcoming: [] },
  ...
  it('does not release OFF when at station platform', () => {
    const snapshot = createMockSnapshot({
      brake: { combined: -0.5, cylinder: 2, effortKn: 40, projectedStopM: 20 },
      speedMs: 2,
      speedDisplay: 4,
      station: { distanceM: 0, nameOcr: 'Test', eta: '' },
      limits: { effective: 25, frontal: 25, next: null, upcoming: [] },
  ...
  it('releases OFF in apply zone when already at target speed', () => {
    ...
    const snapshot = createMockSnapshot({
      brake: { combined: -0.5, cylinder: 2, effortKn: 40, projectedStopM: 120 },
      speedMs: 22.35,
      speedDisplay: 50,
  ...
  it('does not re-brake after OFF coast latch near speed limit', () => {
    ...
    const braking = createMockSnapshot({
      brake: { combined: -0.5, cylinder: 2, effortKn: 40, projectedStopM: 80 },
      speedMs: 22.8,
      speedDisplay: 51,
    ...
    const coasting = createMockSnapshot({
      brake: { combined: 0, cylinder: 0, effortKn: 0, projectedStopM: 200 },
      speedMs: 22.8,
      speedDisplay: 51,
  ...
  it('allows braking again after coast latch when clearly above target', () => {
    ...
    const fastAgain = createMockSnapshot({
      brake: { combined: 0, cylinder: 0, effortKn: 0, projectedStopM: 300 },
      speedMs: 31,
      speedDisplay: 69,
  ...
  it('brakes for speed limit when past nominal apply point (late distStart)', () => {
    ...
    const snapshot = createMockSnapshot({
      brake: { combined: 0.4, cylinder: 0, effortKn: 0, projectedStopM: 900 },
      speedMs: 27.8,
      speedDisplay: 100,
